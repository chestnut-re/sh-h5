...
const TravelCard: FC<TravelProps> = (props) => {
  ...
  useEffect(() => {
    const Stringval = `shtravel://app?data=${encodeURIComponent(
      JSON.stringify({ orderId: orderId, suborderId: id, type: 'verifications' })
    )}`
  ...
  return (
    <div className="trave-content">
      <div className="trave-header">出行确认码</div>
      <div className="trave-title">{StateMap[newstate].qrName}</div>
      <div className="qrcode-content">
        <QRCode value={qrCodeVal} fgColor={newstate != 3 && refundState != 3 ? 'rgba(0,0,0,0.1)' : '#000'} size={100} />
        {newstate != 3 && refundState != 3 ? (
          <div className={`qrcode-state ${StateMap[newstate].cName}`}>{StateMap[newstate].name}</div>
        ) : null}
      </div>
      <div className="trave-name">{travelerName}</div>
    </div>
  )
}
